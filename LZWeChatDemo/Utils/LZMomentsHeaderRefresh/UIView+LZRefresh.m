...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    //屏蔽掉全非状态时的操作
    if (self.refreshStatus == LZRefreshStateNone) {
        return;
    }
    ...
    if (self.refreshStatus == 0) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            self.refreshStatus = LZRefreshStateDefault;
        });
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            /**非刷新状态**/
            if (self.refreshStatus == LZRefreshStateDefault) {
                [self defaultHandleWithOffSet:offsetY change:change];
                
                /**刷新状态**/
            } else if (self.refreshStatus == LZRefreshStateBeginRefresh) {
...
- (void)defaultHandleWithOffSet:(CGFloat)offsetY change:(NSDictionary<NSKeyValueChangeKey,id> *)change {
    ...
    if (defaultoffsetY > self.threshold && defaultoffsetY < 0) {
        ...
        [self anmiationHandelwithChange:change
                              andStatus:LZRefreshStateDefault
                          needAnimation:YES];
    }
    ...
    if (defaultoffsetY <= self.threshold && self.refreshView.contentOffset.y == self.threshold) {
        if (self.extenScrollView.isDragging) {
            ...
            //刷新状态动作处理
            [self anmiationHandelwithChange:change
                                  andStatus:LZRefreshStateBeginRefresh
                              needAnimation:YES];
            // 由非刷新状态 进入 刷新状态
            [self beginRefresh];
        }
...
- (void)beginRefresh {
    //状态取反 保证一次刷新只执行一次回调
    if (self.refreshStatus != LZRefreshStateBeginRefresh) {
        self.refreshStatus = LZRefreshStateBeginRefresh;
 ...
 */
- (void)anmiationHandelwithChange:(NSDictionary<NSKeyValueChangeKey,id> *)change andStatus:(LZRefreshState)status needAnimation:(BOOL)need {
    if (!need) {
        return;
    }
    ...
    /**
     非刷新状态下的动作处理
     */
    if (status == LZRefreshStateDefault) {
        ...
        /**
         刷新状态下的动作处理
         */
    } else if (status == LZRefreshStateBeginRefresh) {
        if (!self.animation) {
...
- (void)endRefresh {
    ...
    if (self.refreshStatus != LZRefreshStateNone) {
        self.refreshStatus = LZRefreshStateNone;
...
- (void)creatMainUI {
    if (!_refreshIcon) {
        _refreshIcon = [[UIImageView alloc]initWithFrame:CGRectMake(10,
                                                                    -10,
                                                                    self.frame.size.width,
                                                                    self.frame.size.height)];
    }
    _refreshIcon.backgroundColor = [UIColor clearColor];
    _refreshIcon.image = [[UIImage imageNamed:@"AlbumReflashIcon"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
